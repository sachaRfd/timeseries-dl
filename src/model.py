...
class TransformerBatchNormEncoderLayer(nn.modules.Module):
    ...
    def forward(
        ...
        is_causal: Optional[Tensor] = None,
    ) -> Tensor:
...
class TSTransformerEncoder(nn.Module):
    def __init__(
        ...
        self.max_len = max_len + 1  # T account for the CLS token.
        self.d_model = d_model
        self.n_heads = n_heads

        self.project_inp = nn.Linear(feat_dim, d_model)
        self.pos_enc = get_pos_encoder(pos_encoding)(
            d_model, dropout=dropout * (1.0 - freeze), max_len=self.max_len
        )
        ...
        # Setup a CLS token:
        self.cls_token = nn.Parameter(torch.randn(1, feat_dim))
    ...
    def forward(self, X, padding_masks):
        ...
        # Add the CLS token to X:
        cls_token = self.cls_token.expand(X.shape[0], 1, -1)
        cls_padding_mask = torch.ones(
            (X.shape[0], 1), dtype=bool, device=padding_masks.device
        )

        X = torch.cat([cls_token, X], dim=1)
        padding_masks = torch.cat([cls_padding_mask, padding_masks], dim=1)
        ...
        # return the reconstructed time-series , and the CLS token:
...
class Predictor_Model(nn.Module):
    ...
    def forward(self, x, padding_mask):
        Z, cls_token = self.encoder(x, padding_mask)

        Z = self.linear_proj_1(cls_token)
